...
function nextAssignment() {
    ...
    if (now < projList.length) {
        if (algorithm == "首次适应算法") {
            if (firstAdapt(memSize, Mem, Board)) { //可以装入作业
                now++;
            } else {
                ;
            }
        } else if (algorithm == "最佳适应算法") {
            if(bestAdapt(memSize, Mem, Board)){
                now++;
            }else{
                ;
            }
...
function firstAdapt(memSize, Mem, Board) {
...
function bestAdapt(memSize, Mem, Board) {
